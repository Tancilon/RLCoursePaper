...
\begin{document}
    ...
    Figure~\ref{fig:learning_curves} shows the learning curve, i.e., the trajectory of performance (mean episodic return) as a function of training steps, which provides a direct visual check of whether the agent improves, slows down, or diverges.

    Under identical settings, we typically observe the ordering $\text{DQN} < \text{Double-DQN} < \text{Dueling-DQN}$ in both sample efficiency and final return. This pattern is consistent with known mechanisms: Double-DQN reduces overestimation bias by decoupling action selection from evaluation, while Dueling separates state value and advantage to improve value estimation when actions are similar, yielding steadier updates, smoother curves, and faster convergence.

    \paragraph{CartPole-v3.}
    All methods eventually solve the task, but vanilla DQN exhibits more oscillation near the success threshold; Double-DQN attenuates these spikes, and Dueling-DQN reaches the optimal regime earlier with a flatter, more stable plateau.

    \paragraph{LunarLander-v3 (discrete).}
    The task is more challenging and exposes instabilities more clearly: DQN shows occasional regressions and slower ascent; Double-DQN substantially stabilizes the rise; Dueling-DQN converges fastest and attains the highest asymptote with the narrowest variability band.

    \paragraph{Evaluation protocol.}
    Curves are computed from mean episodic return on periodic evaluation rollouts (with exploration disabled) and displayed with smoothing and confidence bands across multiple random seeds; earlier and higher plateaus indicate better sample efficiency and stability.
